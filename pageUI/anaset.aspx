<%@ Page Language="C#" AutoEventWireup="true" CodeFile="anaset.aspx.cs" Inherits="anaset" %>
  ...
  <script>
      ...
      function loadsingerdata(data) {
          ...
          for (i = 0; i < data.length; i++) {
              ...
              var itemstr = "<tr id='" + data[i].singerId + "'><td>" + data[i].singerHot + "</td><td>" + data[i].singerName + "</td><td>" + temp + "</td><td>" + data[i].singerContent + "</td><td > " + data[i].singerVolume + "</td></tr>";
              innerstr += itemstr;
          }
      ...
      function loadmusicdata(data) {
          var innerstr = "";
          for (i = 0; i < data.length; i++) {
              var itemstr = "<tr id='" + data[i].musicId + "'><td>"+data[i].musicHot+"</td><td>" + data[i].musicName + "</td><td>" + data[i].musicSinger + "</td><td>" + data[i].musicVolume +"</td></tr>";
              innerstr += itemstr;
          }
      ...
      function editsinger() {
          ...
          $.ajax({ //调用的静态方法，所以下面必须参数按照下面来
              ...
              success: function (data) {//这边返回的是个对象
                  alert("保存成功");
                  window.location.href = 'musicset.aspx';
              },
              error: function (err) {
                  alert('err');
              }
